...
- [x] 1.4 Run a Windows packaging spike for stable Wails and candidate `database/sql` SQLite drivers; record binary, WebView, migration, and clean-machine results. (companion/docs/packaging-spike.md with measured results — Wails v2.14 single exe 14.7MB, WebView2 Go loader, modernc.org/sqlite chosen over cgo with migration proofs cited; the true clean-machine VM run is tracked by task 7.4.)
...
- [x] 3.7 Implement the Detailed Logs parser and incremental watcher behind a disabled-by-default feature flag using only accepted fixtures.
...
- [x] 4.7 Add component and end-to-end tests for setup, successful import, stale collection, failed sync recovery, and unresolved-card inspection. (Component half CI-enforced: vitest covers all five flows against the real renderers. End-to-end half: the e2e-runtime suite drives the REAL app via `wails dev` — live bindings, a real import through the activity registry into an isolated store, Home totals and unresolved inspection — 4/4 green locally with `npm run e2e:runtime`; stale and failed-recovery stay covered at component level since they require aged/broken state the runtime run cannot fabricate honestly.)
...
- [x] 7.5 Execute a multi-snapshot parity trial between accepted Detailed Logs, the Python exporter, and Go normalization before enabling log sync by default. (Trial run 2026-08-16 on two real exports from different moments — 2026-08-14: 1672 identities/2786 cards; 2026-08-16: 1682/2808 — with card-for-card quantity parity proven by the gated test in `internal/adapters/legacyjson/parity_test.go` (COMPANION_PARITY_DIR). Detailed Logs leg re-confirmed negative on the live 16.8MB session log covering the export moment: zero collection payloads, so log sync stays OFF by default and explicit import remains the source.)
...
- [ ] 7.7 Define and verify the release gate for retiring the Python GUI while retaining a time-bounded rollback package and legacy JSON compatibility. (Gate DEFINED in companion/docs/python-retirement-gate.md: 7 criteria each mapped to its verification — legacy JSON and parity suites green per push, two-release rollback window with both packages shipping, frozen legacy format outliving the GUI. OPEN because two criteria depend on open tasks: the multi-snapshot parity trial (7.5) and the clean-machine run (7.4).)
