...
- [x] 8.1 Run Stage 1 manifest and telemetry load tests at 10k-DAU assumptions with p95, error, pool, queue, CPU, I/O, and cost evidence. (CI proof: run 31965523603, internal/loadtest 11.261s — 2000 manifest reads + 400 telemetry batches against the real handlers and a real PostgreSQL container, with p95 budgets (100ms/250ms), zero-error and pool-cap assertions all enforced by the test itself; assumptions, cost extrapolation and honest CI-scale limits pinned in docs/load-stage1.md. Full-scale staging runs stay with 8.2/8.3/8.5.)
- [x] 8.2 Run Stage 2 tests only after Stage 1 and verify 700 events/s burst plus at least 30 percent PostgreSQL headroom. (CI proof: run 31965830554 green — the suite itself asserts the measured rate ≥ 700 events/s over 2500 real batches AND that a 5ms sampler never catches more than 70% of the 20-connection Capacity budget acquired; zero errors required. CI-scale caveats in docs/load-stage1.md; the staging-scale rerun stays with 8.5.)
...
- [x] 8.4 Run CodeQL, dependency, secret, container, migration, API abuse, SSRF, injection, authorization, and production-MCP absence checks. (security workflow green: CodeQL go+python and full-history gitleaks all success; dependency via govulncheck, migration preflight suites, abuse/SSRF/injection/authz batteries and production-MCP absence gates already run per push. Container scan has no target — this repo ships no container images.)
