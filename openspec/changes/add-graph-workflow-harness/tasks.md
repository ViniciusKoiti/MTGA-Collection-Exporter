...
- [x] 4.2 Implement `meta-deck-recommendation` from snapshot and catalog loading through matching, ranking, gaps, and evidence.
...
- [x] 5.3 Implement controlled clock, ID, planner, provider, process, central API, and effect adapters through production ports.
...
- [x] 5.6 Add sanitized small, large, malformed, duplicate-printing, unknown-card, and nearly-buildable-deck fixtures.
...
- [x] 7.6 Add MCP contract and abuse tests for malformed arguments, arbitrary SQL, shell requests, SSRF, oversized results, and disconnects.
...
- [x] 8.4 Run Playwright end-to-end scenarios against Wails progress, approval, cancellation, recovery, and error states. (e2e-runtime suite 6/6 against the REAL app via `wails dev`: typed progress events during a real import, the approval flow pausing on the immutable payload preview, denial surfacing the stable approval_denied error state with nothing written, and approval dispatching the outbox write. Cancellation stays at the engine suite (TestCancelamentoDoContextoEncerraRun) because the UI offers no cancel affordance yet — the e2e follows the control when it exists; recovery is covered by the engine/dual-worker suites and the component-level retry flow.)
